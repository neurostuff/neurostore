...
class ConditionSchema(SQLAlchemyAutoSchema):
...
class PointValueSchema(SQLAlchemyAutoSchema):
...
class AnalysisSchema(SQLAlchemyAutoSchema):
    ...
    id = fields.String(attribute="IRI", data_key="id")
    study = fields.Pluck('StudySchema', "id", dump_only=True)
    condition = 
...
class StudySchema(SQLAlchemyAutoSchema):
    ...
    class Meta:
        model = Study
    #analyses = Nested(AnalysisSchema, data_key='analysis', many=True,
    #                  load_only=True, only=('id',))
    analyses = Nested(AnalysisSchema, many=True)
    id = fields.String(attribute="IRI", data_key="id")


class DatasetSchema(SQLAlchemyAutoSchema):
    def __init__(self, url_root, **kwargs):
...
# #     analysis = StringOrNested(AnalysisSchema, attribute='analyses',
# #                               many=True, dump_only=True)
# #     # analysis = fields.Function(lambda study: study.analysis.IRI,
# #     #                           many=True)
...
# #     @pre_load
# #     def get_nested_schema(self, data, **kwargs):
# #         pass
...
#     analysis = fields.Nested(AnalysisSchema, only=('id',), many=True)
