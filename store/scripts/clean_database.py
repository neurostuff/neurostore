    ...
    if pmid or query:
        try:
            if pmid:
                ...
                handle = Entrez.elink(
                    dbfrom="pubmed", db="pubmed", id=pmid, cmd="neighbor_history"
                )
                ...
                if articles:
                    ...
                    if "Abstract" in article_data:
                        result["description"] = article_data["Abstract"].get(
                            "AbstractText", [""]
                        )[0]
...
def merge_studies(target_bs, studies_to_merge, study_metadata, dry_run=True):
    ...
    try:
        ...
        merged_metadata = {
            ...
            "year": (
                target_bs.year if target_bs.year and target_bs.year > 1900 else None
            ),
        ...
        if merged_metadata["name"] and not target_bs.name:
            print(
                f"Updating name from '{target_bs.name}' to '{merged_metadata['name']}'"
            )
        ...
        for version in all_versions:
            ...
            if needs_moving:
                print(
                    f"Moving version {version.id} from study {version.base_study_id} to {target_bs.id}"
                )
        ...
        for study in studies_to_merge:
            ...
            committed_versions = {
                v for v in changes["to_commit"] if v in study_versions
            }
            unhandled_versions = study_versions - committed_versions

            if unhandled_versions:
                print(
                    f"Study {study.id} has {len(unhandled_versions)} unhandled versions - cannot delete:"
                )
                for v in unhandled_versions:
                    print(f"  Version {v.id} not marked as moved/updated")
            else:
                moved_versions = all(
                    v.base_study_id == target_bs.id for v in study_versions
                )
                if moved_versions:
                    print(
                        f"Study {study.id} verified - all {len(study_versions)} versions marked as moved"
                    )
                    changes["to_delete"].append(study)
                else:
                    print(
                        f"Study {study.id} has versions not moved to target study - cannot delete"
                    )
        ...
        if dry_run:
            ...
            for study in list(changes["to_delete"]):
                ...
                if len(committed_versions) != len(study_versions):
                    print(
                        f"Dry run check: Removing study {study.id} from deletion - versions mismatch"
                    )
        ...
        try:
            with db.session.no_autoflush:
                ...
                version_updates = [
                    obj for obj in changes["to_commit"] if isinstance(obj, Study)
                ]
                ...
                for study in list(changes["to_delete"]):
                    ...
                    updated_versions = {
                        v for v in version_updates if v in study_versions
                    }
                    if len(updated_versions) != len(study_versions):
                        print(
                            f"Warning: Study {study.id} has versions not in update list - skipping deletion"
                        )
                        changes["to_delete"].remove(study)
                        continue
                    ...
                    if any(v.base_study_id != target_bs.id for v in study_versions):
                        print(
                            f"Warning: Study {study.id} has versions not moved to target - skipping deletion"
                        )
                        changes["to_delete"].remove(study)
                        continue
...
def check_duplicates(bs, doi=None, pmid=None, pmcid=None):
    ...
    print(
        f"Target identifiers: PMID={best_target.pmid}, DOI={best_target.doi}, PMCID={best_target.pmcid}"
    )
    ...
    for study in others:
        print(
            f"Study {study.id}: score={score_study(study)}, versions={len(study.versions)}"
        )
    ...
    for bs in no_ids:
        ...
        if best_target:
            ...
            if best_target.id != bs.id:
                print(
                    f"Using study {best_target.id} as merge target (has fuller metadata)"
                )
