...
def update_orphan_studies():
    ...
    orphan_studies = (
        Study.query.filter(Study.base_study_id == None)
        .filter(or_(Study.pmid != None, Study.doi != None, Study.pmcid != None))
        .all()
    )
    ...
    for study in orphan_studies:
        ...
        identifiers = {"pmid": study.pmid, "doi": study.doi, "pmcid": study.pmcid}
