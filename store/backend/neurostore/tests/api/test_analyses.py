...
async def test_get_nested_and_not_nested_analyses(
    auth_client, ingest_neurosynth, session
):
    analysis_id = Analysis.query.first().id
    non_nested = await auth_client.get(f"/api/analyses/{analysis_id}?nested=false")
    nested = await auth_client.get(f"/api/analyses/{analysis_id}?nested=true")

    assert isinstance(non_nested.json()["points"][0], str)
    assert isinstance(nested.json()["points"][0], dict)


async def test_get_analyses(auth_client, ingest_neurosynth, session):
    # List of analyses
    ...
    resp = await auth_client.get(f"/api/analyses/{a_id}")
    resp_json = resp.json()
    assert resp.status_code == 200
...
async def test_get_analyses_filter_by_study(auth_client, session):
    ...
    second_study_resp = await auth_client.post(
        "/api/studies/",
        data={
    ...
    filtered_resp = await auth_client.get(f"/api/analyses/?study={first_study_id}")
    second_filtered_resp = await auth_client.get(
        f"/api/analyses/?study={second_study_id}"
    )
...
async def test_analysis_emits_all_media_flags(auth_client, session):
    ...
    response = await auth_client.get(f"/api/analyses/{analysis_id}")
...
async def test_post_analyses(auth_client, ingest_neurosynth, session):
    ...
    for k in ["user", "id", "created_at", "updated_at", "entities"]:
        analysis.pop(k, None)
    resp = await auth_client.post("/api/analyses/", data=analysis)

    assert resp.status_code == 200


async def test_delete_coordinate_analyses(auth_client, ingest_neurosynth, session):
    analysis_db = Analysis.query.first()
    analysis = AnalysisSchema().dump(analysis_db)
    id_ = auth_client.username
    user = User.query.filter_by(external_id=id_).first()
    ...
    await auth_client.delete(f"/api/analyses/{analysis_db.id}")

    for point in analysis["points"]:
        assert Point.query.filter_by(id=point).first() is None


async def test_delete_image_analyses(auth_client, ingest_neurovault, session):
    analysis_db = Analysis.query.first()
    analysis = AnalysisSchema().dump(analysis_db)
    id_ = auth_client.username
    user = User.query.filter_by(external_id=id_).first()
...
async def test_update_points_analyses(auth_client, ingest_neurovault, session):
    ...
    await auth_client.get(f"/api/analyses/{analysis_db.id}?nested=false")
    await auth_client.get(f"/api/analyses/{analysis_db.id}?nested=true")
    await auth_client.get(f"/api/analyses/{analysis_db.id}")

    update_points = await auth_client.put(
        f"/api/analyses/{analysis_db.id}", data=payload
    )
    ...
    nested_get = await auth_client.get(f"/api/analyses/{analysis_db.id}?nested=false")
    nonnested_get = await auth_client.get(f"/api/analyses/{analysis_db.id}?nested=true")
    get = await auth_client.get(f"/api/analyses/{analysis_db.id}")
...
async def test_post_analysis_without_order(auth_client, ingest_neurosynth, session):
...
async def test_put_analysis_partial_does_not_reset_order(auth_client, session):
    id_ = auth_client.username
    user = User.query.filter_by(external_id=id_).first()
    study = Study(
    ...
    resp = await auth_client.put(
        f"/api/analyses/{analysis_id}", data={"name": "renamed"}
    )
...
async def test_create_duplicate_analysis(auth_client, ingest_neurosynth, session):
    # Get an existing analysis from the database
    analysis_db = Analysis.query.first()
    analysis = AnalysisSchema().dump(analysis_db)
    id_ = auth_client.username
    ...
    resp_duplicate = await auth_client.post("/api/analyses/", data=analysis)
...
async def test_post_analyses_without_order_increments_within_study(
    auth_client, session
):
    ...
    resp1 = await auth_client.post("/api/analyses/", data=dict(payload))
    resp2 = await auth_client.post("/api/analyses/", data=dict(payload))
