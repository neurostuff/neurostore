...
async def test_study_tables_are_ids_only(auth_client, ingest_neurosynth, session):
    ...
    resp = await auth_client.get(f"/api/studies/{study.id}?nested=false")
    assert resp.status_code == 200
    tables = resp.json()["tables"]
    assert all(isinstance(t, str) for t in tables)
    assert set(tables) == {t.id for t in study.tables}

    nested_resp = await auth_client.get(f"/api/studies/{study.id}?nested=true")
    assert nested_resp.status_code == 200
    assert set(nested_resp.json()["tables"]) == set(tables)


async def test_tables_endpoint_lists_analyses(auth_client, ingest_neurosynth, session):
    ...
    resp = await auth_client.get(f"/api/tables/{table.id}?nested=false")
    assert resp.status_code == 200
    assert all(isinstance(a, str) for a in resp.json()["analyses"])

    nested_resp = await auth_client.get(f"/api/tables/{table.id}?nested=true")
    assert nested_resp.status_code == 200
    nested_ids = {a["id"] for a in nested_resp.json()["analyses"]}
    assert nested_ids == {a.id for a in table.analyses}


async def test_table_t_id_uniqueness_per_study(auth_client, ingest_neurosynth, session):
    study = Study.query.first()
    user = User.query.filter_by(external_id=auth_client.username).first()
    _assign_user_to_study_tree(study, user)
    session.commit()
    ...
    resp = await auth_client.post("/api/tables/", data=payload)
    assert resp.status_code == 200

    duplicate = await auth_client.post("/api/tables/", data=payload)
    assert duplicate.status_code == 422


async def test_analysis_requires_table_from_same_study(
...
async def test_table_label_field(auth_client, ingest_neurosynth, session):
    ...
    get_resp = await auth_client.get(f"/api/tables/{table_id}")
