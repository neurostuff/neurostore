...
async def test_study_tables_are_ids_only(async_auth_client, ingest_neurosynth, session):
    ...
    resp = await async_auth_client.get(f"/api/studies/{study.id}?nested=false")
    assert resp.status_code == 200
    tables = resp.json()["tables"]
    assert all(isinstance(t, str) for t in tables)
    assert set(tables) == {t.id for t in study.tables}

    nested_resp = await async_auth_client.get(f"/api/studies/{study.id}?nested=true")
...
async def test_tables_endpoint_lists_analyses(async_auth_client, ingest_neurosynth, session):
    ...
    resp = await async_auth_client.get(f"/api/tables/{table.id}?nested=false")
    assert resp.status_code == 200
    assert all(isinstance(a, str) for a in resp.json()["analyses"])

    nested_resp = await async_auth_client.get(f"/api/tables/{table.id}?nested=true")
    assert nested_resp.status_code == 200
...
async def test_table_t_id_uniqueness_per_study(async_auth_client, ingest_neurosynth, session):
    ...
    resp = await async_auth_client.post("/api/tables/", data=payload)
    assert resp.status_code == 200

    duplicate = await async_auth_client.post("/api/tables/", data=payload)
...
async def test_table_label_field(async_auth_client, ingest_neurosynth, session):
    ...
    get_resp = await async_auth_client.get(f"/api/tables/{table_id}")
