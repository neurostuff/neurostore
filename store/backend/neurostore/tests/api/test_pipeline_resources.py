...
async def test_read_pipelines(auth_client, pipeline1, pipeline2):
    """Test reading list of pipelines."""
    response = await auth_client.get("/api/pipelines/")
...
async def test_read_single_pipeline(auth_client, pipeline1):
    """Test reading a single pipeline by ID."""
    response = await auth_client.get(f"/api/pipelines/{pipeline1.id}")
    assert response.status_code == 200
...
async def test_read_nonexistent_pipeline(auth_client):
    """Test reading a pipeline that doesn't exist."""
    response = await auth_client.get("/api/pipelines/99999")
    assert response.status_code == 404


async def test_read_pipeline_configs(auth_client, result1, result2, result3):
    """Test reading pipeline configurations with optional pipeline filter."""
    response = await auth_client.get("/api/pipeline-configs/")
    assert response.status_code == 200
...
async def test_read_single_pipeline_config(auth_client, pipeline_study_result_payload):
    """Test reading a single pipeline config."""
    # Get config ID from the payload
    config_id = pipeline_study_result_payload[0]["config_id"]

    response = await auth_client.get(f"/api/pipeline-configs/{config_id}")
    assert response.status_code == 200
...
async def test_combined_filters(auth_client, result1, result2, result3):
...
async def test_feature_display_filter(auth_client, result1, result2, result3):
...
async def test_pipeline_config_with_schema(auth_client, pipeline1):
    ...
    response = await auth_client.get(f"/api/pipeline-configs/{pipeline_config.id}")
    assert response.status_code == 200
...
async def test_post_pipeline_study_results_with_study_ids(
    ...
    response2 = await auth_client.post(url, data=payload)
...
async def test_get_pipeline_embeddings_list(auth_client, ingest_demographic_features):
    """Test GET /api/pipeline-embeddings/ returns embeddings created by ingestion."""
    response = await auth_client.get("/api/pipeline-embeddings/")
    assert response.status_code == 200
...
async def test_get_pipeline_embedding_by_id(auth_client, ingest_demographic_features):
    """Test GET /api/pipeline-embeddings/{id} returns the correct embedding."""
    list_resp = await auth_client.get("/api/pipeline-embeddings/")
    assert list_resp.status_code == 200
    list_data = list_resp.json()
    assert list_data["results"]

    emb_id = list_data["results"][0]["id"]
    resp = await auth_client.get(f"/api/pipeline-embeddings/{emb_id}")
...
async def test_filter_pipeline_configs(auth_client, ingest_demographic_features):
    """Test filtering pipeline configs that have embeddings."""
    response = await auth_client.get("/api/pipeline-configs/?has_embeddings=true")
    assert response.status_code == 200
    ...
    response = await auth_client.get("/api/pipeline-configs/?has_embeddings=false")
    assert response.status_code == 200
    data = response.json()
