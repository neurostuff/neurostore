...
class BaseStudy(BaseMixin, db.Model):
    ...
    superceded_by = db.Column(db.Text, db.ForeignKey("base_studies.id"), nullable=True)
    _ts_vector = db.Column(
        "__ts_vector__",
        TSVector(),
        db.Computed(
    ...
    # self-referential relationship for superceded_by
    superceded_by_study = relationship(
        "BaseStudy",
        remote_side="BaseStudy.id",
        foreign_keys=[superceded_by],
    ...
    __table_args__ = (
        ...
        db.CheckConstraint("id != superceded_by", name="no_self_reference"),
