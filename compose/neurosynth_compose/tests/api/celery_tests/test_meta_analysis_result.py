...
def test_meta_analysis_result(app, auth_client, user_data, meta_analysis_results):
    ...
    assert len(data['neurovault_collection']['files']) == len(post_result.json['neurovault_collection']['files'])
...
@celery_test
def test_send_task_file_upload_neurovault(app, db, session, user_data, meta_analysis_results, mock_pynv):# mock_pynv):
    user = User.query.filter_by(name="user1").first()
    meta_analysis = MetaAnalysis.query.filter_by(user=user).first()
    meta_analysis_result = MetaAnalysisResult(meta_analysis=meta_analysis)
    coll_id = 12345
