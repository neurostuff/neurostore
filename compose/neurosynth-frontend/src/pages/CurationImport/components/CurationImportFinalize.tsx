...
import { useProjectCurationColumns, useProjectId, useUpdateCurationColumns } from 'pages/Project/store/ProjectStore';
...
const CurationImportFinalize: React.FC<{
    ...
    const { onNavigate, stubs, unimportedStubs, importMode } = props;
    ...
    const onFinalizeImport = (stubs: ICurationStubStudy[], duplicates: IDuplicateCase[]) => {
        ...
        duplicates.forEach(({ importedStub, projectDuplicates }) => {
            ...
            projectDuplicates.forEach((projectDuplicateStub) => {
                if (projectDuplicateStub.columnIndex === undefined || projectDuplicateStub.studyIndex === undefined) {
                    return;
                }

                const updatedStubStudies = [...updatedColumns[projectDuplicateStub.columnIndex].stubStudies];

                if (projectDuplicateStub.columnIndex > 0 && projectDuplicateStub.resolution === 'duplicate') {
                    ...
                    const { columnIndex, studyIndex, resolution, colName, ...stub } = projectDuplicateStub;
        ...
        enqueueSnackbar('Added new import', {
            variant: 'success',
        });
