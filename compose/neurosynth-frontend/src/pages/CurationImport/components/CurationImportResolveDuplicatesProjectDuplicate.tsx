...
import CurationImportResolveDuplicatesReadOnlyStubSummary from 'pages/CurationImport/components/CurationImportResolveDuplicatesReadOnlyStubSummary';
import CurationImportResolveDuplicates from './CurationImportResolveDuplicates.styles';

type IResolveDuplicatesProjectCurationStubStudy = ICurationStubStudy & {
    columnIndex?: number;
    studyIndex?: number;
...
const CurationImportResolveDuplicatesProjectDuplicate: React.FC<{
    index: number;
    importedStub: ICurationStubStudy & {
        index: number;
        resolution?: 'duplicate' | 'not-duplicate';
    };
    projectDuplicates: IResolveDuplicatesProjectCurationStubStudy[];
    isExpanded: boolean;
    ...
    return (
        <Accordion elevation={0} expanded={isExpanded} onChange={() => onExpand(index)}>
            ...
            <AccordionDetails>
                ...
                <Box sx={CurationImportResolveDuplicates.resolutionContainer}>
                    <Box sx={[CurationImportResolveDuplicates.studyContainer]}>
                        <CurationImportResolveDuplicatesReadOnlyStubSummary {...importedStub} />
                    </Box>
                    <Box sx={{ width: '310px' }}>
                        <ToggleButtonGroup
                            exclusive
                ...
                {projectDuplicates.map((stub, duplicateStubIndex) => (
                    <Box
                        key={stub.id}
                        sx={[
                            CurationImportResolveDuplicates.resolutionContainer,
                            { marginBottom: '0.5rem' },
                        ]}
                    >
                        <Box sx={[CurationImportResolveDuplicates.studyContainer]}>
                            <Chip size="small" color="info" label={`Status: ${stub.colName}`} />
                            <CurationImportResolveDuplicatesReadOnlyStubSummary {...stub} />
                        </Box>
...
export default CurationImportResolveDuplicatesProjectDuplicate;
