...
const CurationDownloadSummaryButton: React.FC<{
    ...
    const handleDownloadIncludedStudies = async (format: 'bibtex' | 'csv') => {
        if (exclusionTags.length === 0) throw new Error('No exclusion tags found');
        setIsLoading(true);

        const date = new Date().toLocaleDateString();
        if (format === 'bibtex') {
            const bibtexStudies = stubsToBibtex(curationColumns, exclusionTags);
            downloadFile(`${projectName}:Curation:${date}.bib`, bibtexStudies, 'text/plain');
        } else {
            const csvStudies = stubsToCSV(curationColumns, exclusionTags);
