...
describe('buildWritableStudyIdMapping.helpers', () => {
    ...
    it('maps each id to itself when old and new snapshots are the same', () => {
        ...
        const idMap = buildClonedStudyIdMap(snapshot, snapshot);
    ...
    it('maps by index and omits unpaired ids when analysis and nested image counts differ', () => {
        const oldSnapshot = buildStudySnapshot(
            'study-old',
            [
                {
                ...
                },
                {
                    ...
                    name: 'B',
                    order: 2,
                    images: [{ id: 'img-old-b1', filename: 'c.nii' }],
                },
            ],
            []
        );
            ...
            [
                {
                    ...
                    name: 'A',
                    order: 1,
                    images: [{ id: 'img-new-a1', filename: 'a.nii' }],
                },
            ],
            []
        );
        ...
        expect(idMap.oldAnalysisIdsToNewIdsMap['analysis-old-1']).toBe('analysis-new-1');
        expect(idMap.oldAnalysisIdsToNewIdsMap['analysis-old-2']).toBeUndefined();
        expect(idMap.oldImageIdToNewIdMap['img-old-a1']).toBe('img-new-a1');
        expect(idMap.oldImageIdToNewIdMap['img-old-a2']).toBeUndefined();
        expect(idMap.oldImageIdToNewIdMap['img-old-b1']).toBeUndefined();
    });
