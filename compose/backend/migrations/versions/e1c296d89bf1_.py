...
def upgrade():
    ...
    if not inspector.has_table("neurostore_analyses"):
        op.create_table(
            ...
            sa.ForeignKeyConstraint(
                ["neurostore_study_id"],
                ["neurostore_studies.neurostore_id"],
            ),
            sa.PrimaryKeyConstraint("id"),
    ...
    if not inspector.has_table("neurovault_files"):
        op.create_table(
            ...
            sa.ForeignKeyConstraint(
                ["collection_id"],
                ["neurovault_collections.collection_id"],
            ),
            sa.PrimaryKeyConstraint("id"),
    ...
    annotation_fk_names = {
        fk.get("name") for fk in inspector.get_foreign_keys("annotations")
    }

    if "cached_studyset_id" not in annotation_columns:
        op.add_column(
            "annotations", sa.Column("cached_studyset_id", sa.Text(), nullable=True)
        )
        op.create_foreign_key(
            None, "annotations", "studysets", ["cached_studyset_id"], ["id"]
        )

    if "annotations_internal_studyset_id_fkey" in annotation_fk_names:
        op.drop_constraint(
            "annotations_internal_studyset_id_fkey", "annotations", type_="foreignkey"
        )
    ...
    meta_fk_names = {
        fk.get("name") for fk in inspector.get_foreign_keys("meta_analyses")
    }
    ...
    if "meta_analyses_internal_studyset_id_fkey" in meta_fk_names:
        op.drop_constraint(
            "meta_analyses_internal_studyset_id_fkey",
            "meta_analyses",
            type_="foreignkey",
        )
    if "meta_analyses_internal_annotation_id_fkey" in meta_fk_names:
        op.drop_constraint(
            "meta_analyses_internal_annotation_id_fkey",
            "meta_analyses",
            type_="foreignkey",
        )

    # Only create new FKs if they are not already present
    existing_meta_fk_targets = {
        (fk["constrained_columns"][0], fk["referred_table"])
        for fk in inspector.get_foreign_keys("meta_analyses")
    }
    if ("cached_annotation_id", "annotations") not in existing_meta_fk_targets:
        op.create_foreign_key(
            None, "meta_analyses", "annotations", ["cached_annotation_id"], ["id"]
        )
    if (
        "neurostore_annotation_id",
        "annotation_references",
    ) not in existing_meta_fk_targets:
        op.create_foreign_key(
            None,
            "meta_analyses",
            "annotation_references",
    ...
    if ("cached_studyset_id", "studysets") not in existing_meta_fk_targets:
        op.create_foreign_key(
            None, "meta_analyses", "studysets", ["cached_studyset_id"], ["id"]
        )
    if (
        "neurostore_studyset_id",
        "studyset_references",
    ...
    results_columns = {
        c["name"] for c in inspector.get_columns("meta_analysis_results")
    }
    ...
    if "meta_analysis_results_neurostore_id_key" in unique_names:
        op.drop_constraint(
            "meta_analysis_results_neurostore_id_key",
            "meta_analysis_results",
            type_="unique",
        )
...
def downgrade():
    ...
    op.add_column(
        "meta_analysis_results",
        sa.Column("neurostore_id", sa.TEXT(), autoincrement=False, nullable=True),
    )
    ...
    op.add_column(
        "meta_analyses",
        sa.Column(
            "internal_studyset_id", sa.TEXT(), autoincrement=False, nullable=True
        ),
    )
    op.add_column(
        "meta_analyses",
        sa.Column(
            "internal_annotation_id", sa.TEXT(), autoincrement=False, nullable=True
        ),
    )
    ...
    op.create_foreign_key(
        "annotations_internal_studyset_id_fkey",
        "annotations",
        "studysets",
        ["internal_studyset_id"],
        ["id"],
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            "status = ANY (ARRAY['OK'::text, 'FAILED'::text, 'PENDING'::text])",
            name="neurovault_file_status_check",
        ),
        sa.ForeignKeyConstraint(
            ["collection_id"],
            ["neurovault_collections.collection_id"],
            name="neurovault_file_collection_id_fkey",
        ),
