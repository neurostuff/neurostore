...
def _post_result(auth_client, meta_analysis, ss_payload, ann_payload):
    ...
    data = {
        "studyset_snapshot": ss_payload,
        "annotation_snapshot": ann_payload,
...
def test_dedup_same_snapshot_one_studyset_row(session, db, auth_client, user_data):
    """POSTing identical studyset_snapshots must converge on a single Studyset row."""
...
def test_snapshot_history_appended_on_result_creation(
    ...
    assert "studyset_snapshot_id" in entry
    assert "annotation_snapshot_id" in entry
...
def test_snapshot_history_entry_per_result_even_for_same_md5(
    ...
    assert all(
        entry.get("studyset_snapshot_id") == canonical_id for entry in entries[-2:]
    )
